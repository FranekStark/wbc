...
AccelerationSceneReducedTSID::AccelerationSceneReducedTSID(RobotModelPtr robot_model,
                                                           QPSolverPtr solver,
                                                           const double dt)
    : Scene(robot_model, solver, dt), hessian_regularizer(1e-8) {
...
const HierarchicalQP& AccelerationSceneReducedTSID::update() {
  if (!configured)
    throw std::runtime_error(
        "AccelerationSceneReducedTSID has not been configured!. PLease call configure() before calling update() for "
        "the first time!");
  ...
  for (auto contraint : constraints[prio]) {
    ...
    if (contraint->type() == Constraint::equality) total_eqs += contraint->size();
    if (contraint->type() == Constraint::inequality) total_ineqs += contraint->size();
    if (contraint->type() == Constraint::bounds) has_bounds = true;
  }
  ...
  for (uint i = 0; i < tasks[prio].size(); i++) {
    ...
    // If the activation value is zero, also set reference to zero. Activation is usually used to switch between
    // different task phases and we don't want to store the "old" reference value, in case we switch on the task again
    ...
    for (int i = 0; i < task->A.cols(); i++) task->Aw.col(i) = joint_weights[i] * task->Aw.col(i);
    ...
    if (task->config.type == wrench_forward) {  // f_ext
      ...
      if (std::find(cp.names.begin(), cp.names.end(), contact_link) == cp.names.end()) {
        LOG_ERROR("Reference frame defined in wrench task %s must match one of the contact points defined robot model",
...
const base::commands::Joints& AccelerationSceneReducedTSID::solve(const HierarchicalQP& hqp) {
  ...
  for (uint c = 0; c < nc; c++)
    tau_out += -robot_model->spaceJacobian(robot_model->worldFrame(), contacts.names[c]).transpose().bottomRows(na)
               * fext_out.segment<3>(c * 3);
...
const TasksStatus& AccelerationSceneReducedTSID::updateTasksStatus() {
  ...
  robot_acc.resize(nj);
  for (size_t i = 0; i < nj; i++) robot_acc(i) = joint_state[i].acceleration;
