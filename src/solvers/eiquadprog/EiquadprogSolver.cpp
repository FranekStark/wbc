...
EiquadprogSolver::EiquadprogSolver() { _n_iter = 100; }
...
void EiquadprogSolver::solve(const wbc::HierarchicalQP& hierarchical_qp, base::VectorXd& solver_output) {
  if (hierarchical_qp.size() != 1)
    throw std::runtime_error(
        "EiquadprogSolver::solve: Constraints vector size must be 1 for the current implementation");
  ...
  eq::EiquadprogFast_status status = _solver.solve_quadprog(qp.H, qp.g, qp.A, -qp.b, _CI_mtx, _ci0_vec, out);
